...
object Day08 extends Problem[Int, Int]:
  def traverseNodes(
    ...
    def traverseAux(currNodes: List[String], dir: List[Char], count: Int): Int =
      if (currNodes.forall(isEndNode)) count
        ...
        traverseAux(
          nextNodes,
          if (dir.tail.isEmpty) directions.toList else dir.tail,
          count + 1
        )
      }

    traverseAux(
      nodes.keySet.filter(isStartingNode).toList,
      directions.toList,
      0
    )
  ...
  def solve(input: String): (Int, Int) =
    ...
    (
      traverseNodes(nodes, instructions, _ == "AAA", _ == "ZZZ"),
      traverseNodes(nodes, instructions, _.endsWith("A"), _.endsWith("Z"))
    )
