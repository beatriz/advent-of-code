  ...
  def solve(input: String): (Int, Int) =
    val (res1, res2) = getLines(input).foldLeft((0, 0)) {
      case ((acc1, acc2), line) =>
        ...
        if (smallRange._1 >= bigRange._1 && smallRange._2 <= bigRange._2)
          (acc1 + 1, acc2 + 1)
        else if (
          (smallRange._1 >= bigRange._1 && smallRange._1 <= bigRange._2) || (smallRange._2 >= bigRange._1 && smallRange._2 <= bigRange._2)
        ) (acc1, acc2 + 1)
        else (acc1, acc2)
    }
    (res1, res2)
