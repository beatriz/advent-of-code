...
class Day05 extends Problem[String, String]:
  ...
  def solve(input: String): (String, String) =
    ...
    val (rearranged1, rearranged2) = instructions.foldLeft((initCrates, initCrates)) {
  ...
  private def parseCrates(cratesRaw: Seq[String]): Map[Int, List[String]] =
    ...
    val (crates, _) = (1 to nCrates).foldLeft((Map.empty[Int, List[String]], cratesStrs.toList)) {
  ...
  case class Instruction(from: Int, to: Int, quantity: Int):
    def move(crates: Map[Int, List[String]], reverse: Boolean): Map[Int, List[String]] =
      val toMove = crates(from).take(quantity)
      val fromPile = crates(from).drop(quantity)
      val toPile: List[String] = (if (reverse) toMove.reverse else toMove) ::: crates(to)
