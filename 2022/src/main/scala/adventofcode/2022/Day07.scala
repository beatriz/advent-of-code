...
class Day07 extends Problem[Long, Long]:
  def solve(input: String) =
    ...
    val smallDirs = dirMap.collect{ case (_, a: Dir) if a.size < 100000 => a.size}
  ...
  def directoriesToDelete(using entities: Map[String, Entity]) =
    ...
    entities.collect{ case (_, dir: Dir) if dir.size >= neededSpace => dir.size}.min


  def buildDirMap(lines: Seq[String]): Map[String, Entity] =
    lines.foldLeft((List.empty[String], Map[String, Entity]("/" -> Dir()))) {
      case ((pathFromHome, acc), line) =>
        line.split(" ") match {
          ...
          case Array("dir", name) => (pathFromHome, updateMapWithEntity(acc, name, pathFromHome, None))
          case Array(size, name) => (pathFromHome, updateMapWithEntity(acc, name, pathFromHome, Some(size.toLong)))
        }
    }._2

  def updateMapWithEntity(map: Map[String, Entity], entName: String, path: List[String], size: Option[Long]) =
    val fullName = (entName :: path).mkString
    val ent = size.fold(Dir())(File(_))
    val currDir = map(path.mkString).asInstanceOf[Dir]
    map.updated(fullName, ent).updated(path.mkString, currDir.addChild(fullName))
